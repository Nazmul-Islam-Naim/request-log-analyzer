{{-- ============================================================
     geo.blade.php  –  Interactive Geo Analytics (world heatmap)
     ============================================================ --}}
@extends('request-log-analyzer::_layout')

@section('title', 'Geo Analytics — Request Log Analyzer')
@section('page-title', 'Geo Analytics')
...
<div class="geo-stats">
    <div class="gs-card">
        ...
        <div class="gs-value" style="color:#2563eb;">{{ number_format($totalCountries) }}</div>
        <div class="gs-sub">unique geo locations</div>
    </div>
    <div class="gs-card">
        <div class="gs-label">Total Requests</div>
        <div class="gs-value">{{ number_format($totalRequests) }}</div>
        <div class="gs-sub">with location data</div>
    </div>
    <div class="gs-card">
        <div class="gs-label">Top Country</div>
        <div class="gs-value" style="font-size:1.1rem;line-height:1.4;">{{ $topCountry?->country ?? '—' }}</div>
        <div class="gs-sub">{{ number_format($topCountry?->count ?? 0) }} requests</div>
    </div>
    <div class="gs-card">
        <div class="gs-label">Top Country Share</div>
        @php $topShare = $totalRequests > 0 ? round(($topCountry?->count ?? 0) / $totalRequests * 100, 1) : 0; @endphp
        <div class="gs-value" style="color:#10b981;">{{ $topShare }}<span style="font-size:1rem;font-weight:600;">%</span></div>
        <div class="gs-sub">of all geo requests</div>
    </div>
</div>

{{-- ── Date filter ──────────────────────────────────────────────────────── --}}
<form method="GET" action="{{ route('request-log-analyzer.geo') }}" class="geo-filter">
    ...
    @if($from || $to)
        <a href="{{ route('request-log-analyzer.geo') }}" class="btn-clear">Clear</a>
    @endif
    @if($from || $to)
...
<div class="geo-table-card">
    ...
    @else
        @php $totalGeo = $countryStats->sum('count'); @endphp
        <div class="tbl-wrap" style="overflow-x:auto;">
            <table class="geo-table" id="geoTable">
                ...
                <tbody>
                    @foreach($countryStats as $i => $cs)
                    @php $pct = $totalGeo > 0 ? round($cs->count / $totalGeo * 100, 2) : 0; @endphp
                    <tr data-country="{{ e($cs->country) }}">
                        <td class="rank">{{ $i + 1 }}</td>
                        <td class="country-name">{{ $cs->country }}</td>
                        <td class="count">{{ number_format($cs->count) }}</td>
                        <td class="bar-cell">
                            <div class="tbl-bar-wrap">
                                <div class="tbl-bar"><div class="tbl-bar-fill" style="width:{{ $pct }}%;"></div></div>
...
(function () {
    ...
    const totalRequests = {{ (int)$totalRequests }};
