{{-- ============================================================
     dashboard.blade.php  –  Overview page  (redesigned)
     ============================================================ --}}
...
<div class="hero-cards">
    ...
    <div class="hc hc-blue">
        <div class="hc-top">
            ...
            <div style="text-align:right;">
                ...
                <div style="font-size:.75rem;color:#374151;font-weight:600;">{{ now()->format('d M') }}</div>
            </div>
        </div>
        <div class="hc-value c-blue">{{ number_format($todayRequests) }}</div>
        <div class="hc-label">Requests Today</div>
        <div class="hc-sub">since midnight · {{ now()->format('T') }}</div>
    </div>
    ...
    <div class="hc hc-green">
        ...
        <div class="hc-value c-green">{{ number_format($activeUsersNow) }}</div>
        <div class="hc-label">Active Users Now</div>
        <div class="hc-sub">within last {{ $windowMinutes }} min window</div>
    </div>
    ...
    <div class="hc hc-purple">
        <div class="hc-top">
            ...
            @if($topRoute)
                <div style="font-size:.65rem;color:#9ca3af;font-weight:600;">{{ number_format($topRoute->count) }} hits</div>
            @endif
        </div>
        @if($topRoute)
            <div class="hc-route" title="{{ $topRoute->uri }}">{{ $topRoute->uri }}</div>
        @else
            <div class="hc-value" style="font-size:1.4rem;color:#d1d5db;">—</div>
        @endif
        <div class="hc-label">Top Route</div>
        <div class="hc-sub">{{ $topRoute ? 'most requested endpoint' : 'no data yet' }}</div>
    </div>
    ...
    <div class="hc hc-amber">
        ...
        <div class="hc-value c-amber">{{ number_format($totalLoggedUsers) }}</div>
...
{{-- ═══════════════════════════════════════════════════════════
     SECTION 2 — Mini metrics strip
═══════════════════════════════════════════════════════════ --}}
<div class="metrics-strip">
    <div class="mc">
        <div class="mc-label">Total Requests</div>
        <div class="mc-value c-blue">{{ number_format($stats['total_requests']) }}</div>
        <div class="mc-sub">all time</div>
    </div>
    <div class="mc">
        <div class="mc-label">Avg Response</div>
        <div class="mc-value c-green">{{ $stats['avg_response_ms'] }}<span style="font-size:.9rem;font-weight:400;color:#9ca3af;"> ms</span></div>
        <div class="mc-sub">mean latency</div>
    </div>
    <div class="mc">
        <div class="mc-label">Error Rate</div>
        <div class="mc-value {{ $stats['error_rate_percent'] > 5 ? 'c-red' : 'c-amber' }}">{{ $stats['error_rate_percent'] }}<span style="font-size:.9rem;font-weight:400;color:#9ca3af;">%</span></div>
        <div class="mc-sub">{{ number_format($stats['error_requests']) }} 4xx/5xx</div>
    </div>
    <div class="mc">
        <div class="mc-label">Exceptions</div>
        <div class="mc-value c-red">{{ number_format($stats['total_errors']) }}</div>
        <div class="mc-sub">captured</div>
    </div>
    <div class="mc">
        <div class="mc-label">DB Queries</div>
        <div class="mc-value c-purple">{{ number_format($stats['total_queries']) }}</div>
        <div class="mc-sub">{{ number_format($stats['slow_queries']) }} slow</div>
    </div>
    <div class="mc">
        <div class="mc-label">Slow Requests</div>
        <div class="mc-value c-red">{{ number_format($stats['slow_requests']) }}</div>
        <div class="mc-sub">&ge; {{ number_format($slowThreshold) }} ms</div>
    </div>
    <div class="mc">
        <div class="mc-label">Req / min</div>
        <div class="mc-value c-blue">{{ $requestsPerMinute }}<span style="font-size:.9rem;font-weight:400;color:#9ca3af;"> /m</span></div>
        <div class="mc-sub">avg over last 60 min</div>
    </div>
</div>

{{-- ═══════════════════════════════════════════════════════════
     SECTION 3 — Charts (2:1 split)
═══════════════════════════════════════════════════════════ --}}
<div class="charts-row">
    ...
    <div class="card">
        <div class="section-head">
            <h2>Top Routes</h2>
            <a href="{{ route('request-log-analyzer.analytics') }}" class="section-head-link">Analytics →</a>
        </div>
        <div class="card-body">
...
{{-- ═══════════════════════════════════════════════════════════     SECTION 3b — Advanced metric charts
═══════════════════════════════════════════════════════ --}}
...
{{-- ═══════════════════════════════════════════════════════     SECTION 4 — Recent Requests (last 10)
═══════════════════════════════════════════════════════════ --}}
<div class="card" style="margin-bottom:1.25rem;">
    <div class="section-head">
        ...
        <a href="{{ route('request-log-analyzer.requests') }}" class="section-head-link">View all →</a>
    </div>
        ...
        <div class="tbl-wrap">
            <table class="req-table">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr>
                        <td style="color:#9ca3af;font-size:.72rem;font-variant-numeric:tabular-nums;">{{ $log->id }}</td>
                        <td><span class="badge {{ $mc }}">{{ $log->method }}</span></td>
                        <td class="uri-cell" title="{{ $log->url }}">{{ $log->uri }}</td>
                        <td class="{{ $log->statusBadgeClass() }}" style="font-weight:600;font-variant-numeric:tabular-nums;">{{ $log->status_code }}</td>
                        <td class="time-cell {{ $tc }}">
                            {{ $log->response_time_ms !== null ? $log->response_time_ms.' ms' : '—' }}
                            @if($log->isSlow())<span class="slow-marker">SLOW</span>@endif
                        </td>
                        <td style="color:#6b7280;text-align:center;">{{ $log->queries_count }}</td>
                        <td style="text-align:center;">
                            @if($log->errors_count > 0)
                                <span class="badge badge-error">{{ $log->errors_count }}</span>
                            @else
                                <span style="color:#d1d5db;">—</span>
                            @endif
                        </td>
                        <td style="color:#9ca3af;font-size:.72rem;white-space:nowrap;">{{ $log->created_at->diffForHumans() }}</td>
                        <td>
                            <div class="action-links">
                                <a href="{{ route('request-log-analyzer.show', $log->id) }}" class="action-link primary">Detail</a>
                                <a href="{{ route('request-log-analyzer.timeline', $log->id) }}" class="action-link">Timeline</a>
                            </div>
                        </td>
                    </tr>
                    @endforeach
    ...
    <div class="card">
        <div class="section-head">
            ...
            @if($recentErrors->isNotEmpty())
                <span class="section-head-badge" style="background:#fef2f2;color:#dc2626;">{{ $recentErrors->count() }}</span>
            @endif
        </div>
        ...
        @else
            ...
            <div class="err-row">
                <div class="err-dot" style="{{ in_array($err->severity, ['critical','alert','emergency']) ? 'background:#9d174d' : '' }}"></div>
                <div class="err-body">
                    <div class="err-class">{{ $err->shortClass() }}</div>
                    <div class="err-msg">{{ $err->message }}</div>
                    <div class="err-meta">
                        <span class="badge badge-{{ $err->severity }}">{{ $err->severity }}</span>
                        <span>{{ $err->created_at->diffForHumans() }}</span>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <div class="card">
        <div class="section-head">
            ...
            @if($slowQueries->isNotEmpty())
                <span class="section-head-badge" style="background:#fef3c7;color:#d97706;">{{ $slowQueries->count() }}</span>
            @endif
        </div>
        ...
        @else
            ...
            <div class="slow-row">
                <div class="slow-ms">{{ number_format($q->time_ms, 1) }} ms</div>
                <div class="slow-sql" title="{{ $q->sql }}">{{ $q->sql }}</div>
                <span class="badge badge-other" style="flex-shrink:0;font-size:.63rem;">{{ $q->connection }}</span>
            </div>
            @endforeach
        @endif
    </div>
...
{{-- ═══════════════════════════════════════════════════════════
     SECTION 6 — Country distribution (conditional)
═══════════════════════════════════════════════════════════ --}}
...
<div class="bottom-row">
    <div class="card">
        <div class="section-head">
            ...
            <a href="{{ route('request-log-analyzer.geo') }}" class="section-head-link" style="margin-left:auto;">View World Map →</a>
        </div>
    ...
    <div class="card">
        ...
        @php $totalGeoHits = $countryStats->sum('count'); @endphp
        <div class="tbl-wrap">
            <table class="req-table">
                <thead><tr><th>Country</th><th>Requests</th><th style="width:140px;">Share</th></tr></thead>
                <tbody>
                    @foreach($countryStats as $cs)
                    @php $sharePct = $totalGeoHits > 0 ? round($cs->count / $totalGeoHits * 100, 1) : 0; @endphp
                    <tr>
                        <td style="font-weight:600;color:#1e293b;">{{ $cs->country }}</td>
                        <td style="font-variant-numeric:tabular-nums;color:#374151;">{{ number_format($cs->count) }}</td>
                        <td>
                            <div class="country-bar-wrap">
                                <div class="country-bar"><div class="country-bar-fill" style="width:{{ $sharePct }}%;"></div></div>
                                <span class="country-pct">{{ $sharePct }}%</span>
                            </div>
                        </td>
                    </tr>
                    @endforeach
...
(function () {
    ...
    const errors  = @json($chartErrors);
    const routes  = @json($topRoutes->pluck('uri'));
    const rcounts = @json($topRoutes->pluck('count'));
    ...
    const avgResponse = @json($chartAvgResponse);
    ...
    const hourLabels   = @json($chartHourLabels);
    const hourlyErrors = @json($chartHourlyErrors);
    ...
    if (miniMapEl) {
        ...
        script.onload = function () {
            ...
            const miniData = @json($countryStats->mapWithKeys(fn($c) => [strtolower($c->country) => (int)$c->count]));
